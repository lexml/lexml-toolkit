...
/**
 * Convert native "item" to oai_dc. In this case, the native "item" is assumed to already be formatted as an OAI <record> element, with the possible exception that
 * multiple metadataFormats may be present in the <metadata> element. The "crosswalk", merely involves pulling out the one that is requested.
 */
public class FileMap2oai_dc extends Crosswalk {
	 ...
	 * 
	 * @param nativeItem
	 *            the native "item". In this case, it is already formatted as an OAI <record> element, with the possible exception that multiple metadataFormats are
