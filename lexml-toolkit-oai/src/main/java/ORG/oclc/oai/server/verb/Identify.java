...
public class Identify extends ServerVerb {
	...
	public static String construct(HashMap context, HttpServletRequest request, HttpServletResponse response, Transformer serverTransformer) throws TransformerException {
		...
		if (hasBadArguments(request, validParamNames.iterator(), validParamNames)) {
			...
			sb.append("<description>");
			LexMLOAI.helper.writePerfil(sb);
			sb.append("</description>");
			...
			sb.append(version);
			sb
					.append("</version><toolkitIcon>http://alcme.oclc.org/oaicat/oaicat_icon.gif</toolkitIcon><URL>http://www.oclc.org/research/software/oai/cat.shtm</URL></toolkit></description>");
